...
import { selectDropdownOptions } from "../../shared/utils/selectDropdownOptions"; // Adjust path if needed
import { selectDynamicDropdownOptions } from "../../shared/utils/selectDynamicDropdownOptions"; // Adjust path if needed
...
export class RootVehiclesPage {
  ...
  async addVehicles(vehicleOrVehicles: Vehicle | Vehicle[]) {
    const vehicles = Array.isArray(vehicleOrVehicles)
      ? vehicleOrVehicles
      : [vehicleOrVehicles];
  ...
  async fillVehicleInfo(vehicle: Vehicle, index: number = 0) {
    ...
    await this.page
      .getByText(`${vehicle.assignedDriver}`, {
        exact: true,
      })
      .click();
    ...
    if (index > 0) {
      await this.page
        .locator(`button[id="input-coverageStatus-${index}"]`)
        .click();
    ...
    await this.page
      .locator(`button[id="input-ownershipType-${index}"]`)
      .click();
    await this.page
      .getByLabel(`${vehicle.ownershipType}`, {
        exact: true,
      })
      .click();
    ...
    if (
      vehicle.garagingAddress?.trim() != null &&
      vehicle.garagingAddress != "8607 Concerto Cir"
    ) {
      await this.page
        .locator(`button[id="input-garagingAddr-${index}"]`)
        .click();
      await this.page.getByLabel(`Different from Primary`).click();
      await this.page.fill(
        `input[id="input-garagingAddr-${index}"]`,
    ...
    if (vehicle.lienholder?.trim() != null && vehicle.ownershipType != "Own") {
      await this.page.fill(
        `input[id="input-lienholderInput-${index}"]`,
        vehicle.lienholder
      );
    }
    ...
    // Anti-Theft dropdown
    await this.page
      .locator(`button[id="input-antiTheftEquipment-${index}"]`)
      .click();

...
export class RootCheckoutPage {
  ...
  private diclaimerCheckbox: Locator;
  private payNowButton: Locator;

  constructor(private page: Page) {
    this.heading = page.getByRole("heading", { name: "Checkout", exact: true });
    this.monthlyPaymentSelector = page.getByRole("button", {
      name: "Next Step",
    });
    this.fullPaymentSelector = page.locator(
      'input[type="radio"][value="Full Pay"]'
    );
    this.cardHolderNameIframe = page.frameLocator(
  ...
  async fillCreditCardInformation(cardDetails: {
    ...
    const cardHolderNameField = this.cardHolderNameIframe.locator(
      "input#cardholder-name"
    );
    await cardHolderNameField.fill(cardDetails.cardHolderName);

    // Fill Card Number
    const cardNumber = this.cardNumberIframe.locator(
    ...
    const cardExpirationDate =
      this.expirationDateIframe.locator("input#expiration");
  ...
  async fillCardHolderName(cardHolderName: string) {
    const frameLocator = this.page.frameLocator(
      ".braintree-card.braintree-form.braintree-sheet iframe"
    );
    await frameLocator
      .locator(".braintree-form-cardholder-name")
  ...
  async checkDisclaimer() {
    await this.diclaimerCheckbox.click();
  }

  async clickPayNow(nextPageCheck: () => Promise<void>) {
